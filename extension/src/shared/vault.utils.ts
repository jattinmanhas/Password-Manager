...
export function parseVaultMetadata(metadata: unknown): VaultMetadata {
  ...
  // If metadata is a Base64 string (due to Go's []byte encoding in JSON), decode it
  if (typeof metadata === "string" && metadata.length > 0) {
    try {
      const decoded = fromBase64(metadata);
  ...
  return {
    ...
    vault_id: typeof obj.vault_id === "string" ? obj.vault_id : undefined,
    vault_name: typeof obj.vault_name === "string" ? obj.vault_name : undefined,
    vault_type: normalizeVaultType(obj.vault_type),
